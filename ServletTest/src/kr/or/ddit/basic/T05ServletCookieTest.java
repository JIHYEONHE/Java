...
/*
 * 쿠키에 대하여 ..
 * 클라이언트에 저장 (서버과부화를 막아줌)하지만 보안이 약함
 * (웹서버와 브라우저는 애플리케이션을 사용하는 동안 필요한 값을 쿠키를 통해 공유하여 상태를 유지함.ㄴ)
 * 1.구성요소 
 *  - 이름 
 *  - 값
 *  - 유효시간(초)
 ...
 *  - 쿠키 전송단계: 웹브라우저는 저장한 쿠키를 요청이 있을때 마다 웹서버에 전송한다.(삭제전까지..)
 *  			 웹 서버는 브라우저가 전송한 쿠키를 사용하여 필요한 작업을 수행한다.
 */
public class T05ServletCookieTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		deleteCookieExam(req, resp); // 쿠기 정보 삭제 예제
	}
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		 ...
		 * => 지정하지 않으면 브라우저 종료시 삭제됨 cookie.setMaxAge(60*60*24);
		 * //24시간
		 * 
		 * 3.응답헤더에 쿠키객체를 추가한다. response.addCookie(cookie);
		 * 
		 * => 출력버퍼가 플러시 된 이후에는 쿠키를 추가할 수 없다. (응답헤더를 통해서 웹브라우저에 전달하기 때문에...)
		 */
