...
class DataBox {
	...
	//data가 null인 경우에만 자료를 세팅하는 메서드
	public synchronized void setData(String data) {
		if(this.data != null) {//세팅하기 위해 데이터가 없어야함 
			try {
			System.out.println(Thread.currentThread().getName()
					+"wait() 호출");
		wait();
...
// 데이터를 세팅만 하는 스레드
class ProducerThread extends Thread{ //데이터를 생산
...
//데이터를 읽어만 오는 스레드 
class ConsumerThread extends Thread{ //데이터를 소비하는 스레드
	private DataBox dataBox;
